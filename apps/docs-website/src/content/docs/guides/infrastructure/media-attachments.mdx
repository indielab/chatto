...
<Aside type="note">
  Signed attachment URLs are bearer capabilities until they expire. The bundled PWA uses same-origin virtual asset URLs where possible so normal page markup does not expose portable remote attachment URLs.
</Aside>
...
<CardGrid>
  ...
  <LinkCard title="Attachment API" href="/reference/connectrpc-api/attachments/" description="Typed API reference for room attachment reads and URL refreshes." />
</CardGrid>
