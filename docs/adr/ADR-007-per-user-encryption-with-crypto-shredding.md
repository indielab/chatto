...
- **Algorithm**: New message bodies use a compact versioned envelope: XChaCha20-Poly1305 encrypts the body with the author's active message-body DEK epoch. Each DEK is generated by Chatto, wrapped through the KMS boundary, stored as a protobuf `UserDataEncryptionKey` wrapped-key record in `ENCRYPTION_KEYS` under an opaque content-key ref, and recorded for audit as a durable `UserDEKGeneratedEvent` containing only refs and wrapping metadata. Legacy bodies encrypted directly with the per-user ChaCha20-Poly1305 key remain readable.
