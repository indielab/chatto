...
func TestPostMessage_ReplyPermissions(t *testing.T) {
	...
	t.Run("member without message.reply denied cannot use inReplyTo in thread", func(t *testing.T) {
		groupID := env.testRoom.GroupId
		if err := env.core.DenyGroupPermission(env.ctx, groupID, core.RoleEveryone, core.PermMessageReply); err != nil {
			t.Fatalf("failed to deny permission: %v", err)
		}
	...
	t.Run("member without message.reply can still post in thread without inReplyTo", func(t *testing.T) {
		groupID := env.testRoom.GroupId
		if err := env.core.DenyGroupPermission(env.ctx, groupID, core.RoleEveryone, core.PermMessageReply); err != nil {
			t.Fatalf("failed to deny permission: %v", err)
		}
