...
- **Use `text-accent hover:underline` for inline links, not `text-primary`.** The "primary" design token resolves to a slate/neutral gray in this theme, so `text-primary` links look indistinguishable from muted body text. `text-accent` (violet) is the canonical link color. The login footer's `text-muted hover:text-primary` pattern is a deliberate exception for subdued small-print links.
