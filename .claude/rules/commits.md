...
- When creating or editing a multiline PR or issue body with `gh`, write the body to a markdown file or stdin and use `--body-file`. Do not pass escaped `\n` sequences to `--body`; they render literally on GitHub. After creating or editing, inspect the stored body with `gh pr view <pr> --json body --jq .body` (or the issue equivalent) before telling the user the PR is ready.
